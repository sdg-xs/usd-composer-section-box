...
_MODEL_PATH = Path(__file__).resolve().parent.parent / "section_box" / "model.py"
...
def check_default_faces() -> str:
    assert SectionBox().faces == frozenset(Face), f"a bare SectionBox enables all 6 faces, got {SectionBox().faces}"
...
def check_for_bounds() -> str:
    ...
    assert box.size == Gf.Vec3d(6.0, 8.0, 10.0), f"size is the full extent of the bounds, got {box.size}"
    ...
    corners = box.corners()
    assert any(point_close(c, (-1.0, -2.0, -3.0)) for c in corners), f"the minimum bound is corner 0, got {corners}"
    assert any(point_close(c, (5.0, 6.0, 7.0)) for c in corners), f"the maximum bound is corner 7, got {corners}"
...
def check_invalid_axis_rejected() -> str:
    ...
    try:
        ...
        return "rotated rejects axis 3"
    raise AssertionError("rotated must reject axis 3, there are only 3 axes")
...
def check_translated() -> str:
    ...
    assert point_close(new_pos, (10.0, 0.0, 0.0)), f"translated shifts the centre by the offset, got {new_pos}"
...
def check_resized() -> str:
    ...
    assert close(grown.size[0], 6.0), f"growing MAX_X by 4 takes width from 2 to 6, got {grown.size[0]}"
    ...
    assert close(new_min_x, old_min_x), f"the opposite face stays fixed: expected {old_min_x}, got {new_min_x}"
    # Shrinking below zero clamps to 0.
    shrunk = box.resized(Face.MAX_X, -100.0)
    assert close(shrunk.size[0], 0.0), f"size clamps to 0 when delta overshoots, got {shrunk.size[0]}"
    assert close(shrunk.transform.ExtractTranslation()[0], old_min_x), (
        "overshooting a resize must keep the opposite face fixed"
    )
    return "resized() grows/shrinks on one axis, opposite face stays fixed"


def check_all_six_planes() -> str:
    box = SectionBox(size=Gf.Vec3d(2, 4, 6)).rotated(1, 37).translated(Gf.Vec3d(11, -7, 4))
    planes = box.active_planes()
    assert len(planes) == 6
    for face, plane in zip(Face, planes):
        normal = Gf.Vec3d(*plane[:3])
        local = model.AXIS_VECTORS[face.axis] * face.sign * box.size[face.axis] * 0.5
        on_face = box.transform.Transform(local)
        assert close(Gf.Dot(normal, on_face) + plane[3], 0)
        outside = on_face + normal
        assert Gf.Dot(normal, outside) + plane[3] > 0
    center = box.transform.ExtractTranslation()
    assert all(Gf.Dot(Gf.Vec3d(*p[:3]), center) + p[3] < 0 for p in planes)
    return "six transformed planes bound the box and reject points beyond every face"


def check_face_setter() -> str:
    box = SectionBox()
    for face in Face:
        removed = box.with_face(face, False)
        assert face not in removed.faces
        assert len(removed.active_planes()) == 5
        assert removed.with_face(face, False) == removed
        assert removed.with_face(face, True) == box
    return "face setters are idempotent and remove only the requested plane"


def check_rotated_resize_clamp() -> str:
    box = SectionBox(size=Gf.Vec3d(2, 4, 6)).rotated(2, 31)
    for face in Face:
        opposite = model.AXIS_VECTORS[face.axis] * -face.sign
        before = box.transform.Transform(opposite * box.size[face.axis] * 0.5)
        after = box.resized(face, -100)
        assert (after.transform.ExtractTranslation() - before).GetLength() < TOLERANCE
